...
class RewardsRepository {
  ...
  Future<RewardsPageData> list(
      {String? category, int page = 1, int limit = 12}) async {
  ...
  Future<List<DataPlan>> getDataPlans(String network) async {
    final res = await _api
        .get('/admin/rewards/data-plans', query: {'network': network});
    final items = (res['items'] as List?) ?? [];
    return items
        .map((e) => DataPlan.fromJson((e as Map).cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<void> outOfStock(String id) =>
      _api.post('/admin/rewards/$id/out-of-stock');
