...
""";
  try {
    ...
    // Test WalletCharts topGames parsing with parens:
    final topGames = ((json['top_games_by_coins'] ?? json['top_games']) as List<dynamic>?)
              ?.map((e) => TopGame.fromJson(e as Map<String, dynamic>))
              .toList() ?? [];
    
    // Test WalletRecent
    final cashTransactions = List<Map<String, dynamic>>.from(json['recent_cash'] ?? []);
    final coinTransactions = List<Map<String, dynamic>>.from(json['recent_coins'] ?? []);
    final pendingRedemptions = List<Map<String, dynamic>>.from(json['pending_redemptions'] ?? []);

    print('Success: ${topGames.length} top games');
  } catch(e, st) {
    print('Error: $e');
  }
}
